...
class MassachusettsSegmentation(Dataset):
    """
    ...
    def __init__(self,
        ...
        """
        :param base_dir: path to Massachusetts dataset directory
        ...
        self.args = args
        self.images = sorted(glob(os.path.join(base_dir, 'tiff', split, "*.tiff")))
        self.labels = sorted(glob(os.path.join(base_dir, 'tiff', f"{split}_labels", "*.tif")))
        # print(self.images[-1])
        # print(self.labels[-1])
    ...
    def __getitem__(self, index):
        ...
        if self.split == "train":
            ...
            # mask = torch.tensor([255]).reshape(1, 1, 1).repeat(3, 513, 513)
            # mask = sample['image'] == mask
            # print(mask.shape)
            # sample['label'][mask] = 0
            # torch.where(sample['image'] == torch.tensor([[255],[255],[255]]), torch.tensor([[0],[0],[0]]))
    ...
    def __str__(self):
        return 'Massachusetts(split=' + str(self.split) + ')'
...
if __name__ == '__main__':
    ...
    train = MassachusettsSegmentation(args, split='train')
